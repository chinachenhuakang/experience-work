...
### 振华任务跟踪模块
  使用环境：网易低代码开发平台Codewave
  开发环境：浏览器
  证件：中级开发工程师认证

#### 项目背景
对任务进行管理，分三层任务，各个级别控制自己任务，负责人把控主任务，通过审批进行有效管理，对任务的各个状态及进度进行相应的人员通知、告知、预警

#### 工作职责
1、工作台中的待完成、已完成事项
2、整体两个模块的业务逻辑及画面还原，通过建立数据库设计，及低代码中的控件然后处理页面样式，写数据业务逻辑，补充业务逻辑，格式转换，设置特定组件风格
3、对于特殊联表查询进行SQL处理

#### 心得
引入了HTML生成PDF插件，jspdf 控制节点的大小展示等比宽高，生成对应的pdf，分页通过计算元素总高进行分页


### 重庆长江建设一体化检测平台项目
  使用环境：Web端
  开发环境：Vue2 +Taco UI; vue2 + uView
  后端环境：SpringBoot

#### 项目背景
检测系统，主要对整个检测流程进行数据化转化管理，有效把控整体及流程的规范化，另外还有小程序端，方便客户进行委托。包含基础数据、委托、样品、检测、设备、资源、质量体系等管理模块，全流程管理

#### 工作职责
1、管理后台的全部业务模块【PC端、小程序端】
2、主要是按需求开发还原UI页面、根据需求按原型开发管理页面
...
#### 项目背景
项目是重庆云阳愁抽蓄管理系统，和粤电一样涉及到文件安全、视频监控、质量管理、安全管理

#### 工作职责
1、开发项目安全管理-双重预防、作业安全【PC端、APP端】
2、主要是按需求开发还原UI页面、根据需求按原型开发管理页面

#### 心得
对多个视频展示，各自视频流控制自己的功能按钮处理，对Ref进行批量定义，使用 memo渲染视频组件。
...
#### 项目背景
广东水利抽蓄工程项目，对各个子项的工作管理、质量管理、安全管理、技术提高工作效率及质量，对线下内容进行数据化，利于管理和统计，分析总结，出现问题及时给出最合理的方案。

#### 工作职责
1、开发项目视频监控管理。对摄像头自动手动抓拍进行处理，形成单据移交到质量或者安全类问题处理
2、开发多项目大屏展示数据统计，对数据进行可视化处理，不同项目展示不同数据；多项目配置功能，对可视化数据配置处理
3、开发监控大屏气体模块的监查，对于异常数据及时报警提示
4、主要是按需求开发还原UI页面、根据需求按原型开发管理页面
